...
void ImportWizardPageSelect::itemSelected(QListWidgetItem* current, QListWidgetItem* previous)
{
    ...
    case ImportWizard::IMPORT_OPVAULT:
        ...
        break;
    // Password and/or Key File Required
...
{
    m_ui->passwordEdit->setEnabled(passwordEnabled);
    m_ui->passwordEdit->setToolTip("");
    if (!passwordEnabled) {
        m_ui->passwordEdit->setToolTip(tr("A password is not required to import this file type."));
    }
    m_ui->keyFileButton->setEnabled(keyFileEnable);
    m_ui->keyFileEdit->setEnabled(keyFileEnable);
    m_ui->keyFileEdit->setToolTip("");
    if (!keyFileEnable) {
        m_ui->keyFileEdit->setToolTip(tr("A key file is not required to import this file type."));
    }
}
