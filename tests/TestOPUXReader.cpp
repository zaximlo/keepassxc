...
QTEST_GUILESS_MAIN(TestOPUXReader)

void TestOPUXReader::initTestCase()
{
    QVERIFY(Crypto::init());

    m_opuxPath = QStringLiteral("%1/%2").arg(KEEPASSX_TEST_DATA_DIR, QStringLiteral("/1PasswordExport.1pux"));
}

void TestOPUXReader::testReadIntoDatabase()
{
    OPUXReader reader;
    QSharedPointer<Database> db(reader.convert(m_opuxPath));
    QVERIFY(db);
